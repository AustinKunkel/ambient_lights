...
def setup(strip):
  ...
  global is_static_color, strip_color, change_brightness, capturing_with_avg
  l_count = int(sc.sc_settings["left-count"])
  r_count = int(sc.sc_settings["right-count"])
  t_count = int(sc.sc_settings["top-count"])
  b_count = int(sc.sc_settings["bottom-count"])

  capt = int(lf.led_values['capt'])
  fx = int(lf.led_values['fx'])
  is_avg_color = int(sc.sc_settings['avg-color'])

  print(f"capt: {capt}, fx: {fx}, avg: {is_avg_color}")
  ...
  if capt == 1 and is_avg_color == 1:
    capturing_with_avg = True
    is_static_color = False
    change_brightness = True
  print(f"is static coloe: {is_static_color}, change brightness: {change_brightness}")
...
def update_led_brightness(strip, start, end, brightness_factor=1, passed_color=None):
  global is_static_color, strip_color, capturing_with_avg
...
def run(indata, strip):
  ...
  - lowcut (int): The low cutoff frequency for the bandpass filter.
  - highcut (int): The high cutoff frequency for the bandpass filter.
  """
  global l_count, r_count, t_count, b_count
  ...
  try:
    ...
    mid_ratio = scale_rms(mid_rms, 50, 2000)
